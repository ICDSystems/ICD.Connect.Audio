...
namespace ICD.Connect.Audio.Biamp.Tesira.AttributeInterfaces.MixerBlocks.GatingAutoMixer
{
	public sealed class GatingAutoMixerChannel : AbstractAttributeChild<GatingAutoMixerBlock>, IVolumeAttributeInterface
	{
		...
		public eDirectOutput DirectOutput
		{
			...
			private set
			{
				...
				Log(eSeverity.Informational, "DirectOutput set to {0}", m_DirectOutput);
		...
		public float GateHoldTime
		{
			...
			private set
			{
				...
				Log(eSeverity.Informational, "GateHoldTime set to {0}", m_GateHoldTime);
		...
		public bool Manual
		{
			...
			private set
			{
				...
				Log(eSeverity.Informational, "Manual set to {0}", m_Manual);
		...
		public bool NomGainEnabled
		{
			...
			private set
			{
				...
				Log(eSeverity.Informational, "NomGainEnabled set to {0}", m_NomGainEnabled);
		...
		public float OffAttenuation
		{
			...
			private set
			{
				...
				Log(eSeverity.Informational, "OffAttenuation set to {0}", m_OffAttenuation);
		...
		{
			Value innerValue = value.GetValue<Value>("value");
			DirectOutput = innerValue.GetObjectValue(s_DirectOutputModes);
		}
